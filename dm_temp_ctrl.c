...
volatile c_temp		g_chain_tmp[ASIC_CHAIN_NUM];	// current temperature per chain

volatile int		g_tmp_last_upd_time[ASIC_CHAIN_NUM];
static uint32_t 	g_temp_status[ASIC_CHAIN_NUM];
...
uint32_t dm_tempctrl_update_chain_temp(int chain_id)
{
	...
	if (!mcompat_get_chain_temp(chain_id, &chain_temp))
		applog(LOG_NOTICE, "chain%d: failed to read chain temperature", chain_id);
#else
...
#endif
	applog(LOG_ERR, "chain%d: Tmax=%d, Tmin=%d, Tavg=%d",
